  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <div>
          <h4 className="font-medium text-gray-800 dark:text-gray-200 mb-1">{task.nombre || task.title}</h4>
          <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 mb-1">
            <Hash className="h-3 w-3 mr-1" />
            <span className="font-mono">{task.codigo}</span>
          </div>
        </div>
        <Button
          variant="ghost"
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        ...
        {task.prerrequisitos && task.prerrequisitos.length > 0 && (
          <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 px-2 py-1 rounded-md">
            <BookOpen className="h-3 w-3 mr-1" />
            {task.prerrequisitos.length} prerreq.
          </div>
        )}
